...
import { sortBySmallestDiff } from './sort';
...
export const findClosestValue = <T extends number>(value: T, values: T[]): T => {
  const [closestValue, ...vals] = [...values].sort(sortBySmallestDiff(value));
  console.log({ value, closestValue, vals })
  return closestValue;
}